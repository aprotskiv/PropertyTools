...
namespace PropertyTools.Wpf
{
	...
	using SelectionChangedEventHandler = System.Action<object, Avalonia.Controls.SelectionChangedEventArgs>;
		...
		, INotifyPropertyChanged
	{
...
#if AVALONIA
			DependencyProperty.Register<ColorPickerPanel, int>(
				nameof(Alpha),
				0, // default value
...
#if AVALONIA
			DependencyProperty.Register<ColorPickerPanel, bool>(
				nameof(IsPicking),
				false, // default value
...
#if AVALONIA
			DependencyProperty.Register<ColorPickerPanel, Color?>(
				nameof(SelectedColor),
				Color.FromArgb(0, 0, 0, 0), // default value
				defaultBindingMode: Avalonia.Data.BindingMode.TwoWay,
				coerce: CoerceSelectedColorValueStatic
#else
			DependencyProperty.Register(
				nameof(SelectedColor),
				typeof(Color?),
				...
				new FrameworkPropertyMetadata(
					...
					CoerceSelectedColorValueStatic)
#endif
			);

		/// <summary>
		/// Identifies the <see cref="ShowHsvPanel"/> dependency property.
		/// </summary>
		public static readonly DependencyProperty ShowHsvPanelProperty =
#if AVALONIA
			DependencyProperty.Register<ColorPickerPanel, bool>(
				nameof(ShowHsvPanel),
				false, // default value
		...
		/// The show hsv panel.
		/// </summary>
		private static bool showHsvPanelStaticFlag;
		...
		public ColorPickerPanel()
		{
			...
			this.ShowHsvPanel = showHsvPanelStaticFlag;
		}
		...
		public bool ShowHsvPanel
		{
			get
			{
				return (bool)this.GetValue(ShowHsvPanelProperty);
			}
			set
			{
				this.SetValue(ShowHsvPanelProperty, value);				
			}
		}
		...
		protected override void OnApplyTemplate(TemplateAppliedEventArgs e)
		{
			base.ApplyTemplate();
			//this.te
			this.hsvControl = e.NameScope.Find(PartHsv) as HsvControl;
			var predefinedColorPanel = e.NameScope.Find(PartPredefinedColorPanel) as StackPanel;
			if (predefinedColorPanel != null)
			{
				predefinedColorPanel.AddHandler(SelectingItemsControl.SelectionChangedEvent,
					(SelectionChangedEventHandler)this.OnPredefinedColorPanelSelectionChanged, 
		...
		protected virtual
#if AVALONIA
			Color? CoerceSelectedColorValue(Color? baseValue)
#else
			object CoerceSelectedColorValue(object baseValue)
#endif
		{
			if (baseValue == null)
			{
		...
		protected override void OnPropertyChanged(Avalonia.AvaloniaPropertyChangedEventArgs change)
		{
			...
			if (change.Property == IsPickingProperty)
			{
				IsPickingChanged(this, change);
			}
			else if (change.Property == ShowHsvPanelProperty)
			{
				showHsvPanelStaticFlag = this.ShowHsvPanel; // update static flag
			}
			else
			{
				OnComponentChanged(change);
			}
		}
#endif
		...
		private static
			...
			object
#endif
		CoerceSelectedColorValueStatic(DependencyObject d,
#if AVALONIA
			Color?
#else
			object
		...
		private void OnPredefinedColorPanelSelectionChanged(object sender, SelectionChangedEventArgs args)
		{
			var listBox =
#if AVALONIA
			args.Source as ListBox;
#else
			args.OriginalSource as ListBox;
#endif
